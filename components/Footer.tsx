"use client"

import { useState } from "react";

// ─── Config — edit these to customise ────────────────────────────────────────

const SITE_NAME = "Forest Vibes";
const SITE_TAGLINE =
  "A sanctuary nestled in nature. Where every breath of fresh air and every rustle of leaves reminds you that you belong here.";

// Set to a real image path/URL to show a logo image, otherwise initials are shown
const LOGO_IMAGE_URL: string | null = "/logo-nobg.webp"; // e.g. "/logo.png" or "https://..."

const quickLinks = [
  { label: "Home", href: "/" },
  { label: "About", href: "/about-us" },
  { label: "Contact", href: "contact-us" },
  { label: "Gallery", href: "#" },
  // { label: "Rooms & Suites", href: "#" },
  // { label: "Restaurant", href: "#" },
  // { label: "Blog", href: "#" },
];

const contactInfo = {
  address: "Moza Bajhet, Khasra no- 51, Bajhet, Raipur PO:Raipur, DIST: Dehradun, Uttarakhand - 248008",
  phone: "+91 7500131319",
  email: "stay@forestvibes.in",
};

const socials = [
  {
    label: "Facebook",
    href: "https://www.facebook.com/share/1NavwWixoM/",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
    ),
  },
  // {
  //   label: "Telegram",
  //   href: "#",
  //   icon: (
  //     <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
  //       <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
  //     </svg>
  //   ),
  // },
  {
    label: "Instagram",
    href: "https://www.instagram.com/forrestvibes.in/",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
      </svg>
    ),
  },
];

// ─── Components ───────────────────────────────────────────────────────────────

function Logo() {
  return (
    <div className="flex items-center gap-3 mb-6">
      {LOGO_IMAGE_URL ? (
        <img
          src={LOGO_IMAGE_URL}
          alt={SITE_NAME}
          className="w-26 h-20 rounded-full object-cover"
        />
      ) : (
        <div className="w-12 h-12 rounded-full bg-[#3a6349] flex items-center justify-center flex-shrink-0">
          <span className="text-white font-serif text-lg font-bold leading-none">
            {SITE_NAME.split(" ").map((w) => w[0]).join("")}
          </span>
        </div>
      )}
      <span
        className="text-[#1a1a1a] text-2xl tracking-tight"
        style={{ fontFamily: "'Georgia', serif", fontStyle: "italic" }}
      >
        {SITE_NAME}
      </span>
    </div>
  );
}

// ─── Footer ───────────────────────────────────────────────────────────────────

export default function Footer() {
  const [email, setEmail] = useState("");
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (email) setSubmitted(true);
  };

  return (
    <footer className="bg-[#f0ede4] font-sans">
      {/* Top divider */}
      <div className="h-px bg-[#d8d4c8] mx-8" />

      <div className="max-w-6xl mx-auto px-6 py-14">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">

          {/* ── Col 1: Brand (5 cols) ── */}
          <div className="md:col-span-5">
            <Logo />
            <p className="text-[13px] text-black leading-relaxed mb-8 max-w-sm">
              {SITE_TAGLINE}
            </p>

            {/* Newsletter */}
            <p className="text-[10px] uppercase tracking-widest text-black font-semibold mb-3">
              Subscribe to our newsletter
            </p>
            {submitted ? (
              <p className="text-[13px] text-[#3a6349] font-semibold">
                Thank you for subscribing!
              </p>
            ) : (
              <form onSubmit={handleSubmit} className="flex">
                <input
                  type="email"
                  placeholder="Your email address"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="flex-1 bg-transparent border border-[#cbc7bc] border-r-0 px-4 py-2.5 text-[13px] text-[#1a1a1a] placeholder-[#aaa] outline-none focus:border-[#3a6349] transition-colors"
                  required
                />
                <button
                  type="submit"
                  className="bg-[#3a6349] text-white text-[11px] font-bold uppercase tracking-widest px-5 py-2.5 hover:bg-[#2d4f39] transition-colors flex-shrink-0"
                >
                  Submit
                </button>
              </form>
            )}

            {/* Socials */}
            <div className="flex gap-4 mt-7">
              {socials.map((s) => (
                <a
                  key={s.label}
                  href={s.href}
                  aria-label={s.label}
                  className="w-9 h-9 border border-[#cbc7bc] flex items-center justify-center text-[#555] hover:border-[#3a6349] hover:text-[#3a6349] transition-colors"
                >
                  {s.icon}
                </a>
              ))}
            </div>
          </div>

          {/* ── Col 2: Quick Links (3 cols) ── */}
          <div className="md:col-span-3">
            <p className="text-[20px] uppercase tracking-widest text-black font-bold mb-6">
              Quick Links
            </p>
            <ul className="flex flex-col gap-3">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
                    className="text-[13px] text-black hover:text-[#3a6349] transition-colors flex items-center gap-2 group"
                  >
                    <span className="w-3 h-px bg-[#cbc7bc] group-hover:bg-[#3a6349] group-hover:w-4 transition-all duration-200 inline-block" />
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* ── Col 3: Contact (4 cols) ── */}
          <div className="md:col-span-4">
            <p className="text-[20px] uppercase tracking-widest text-black font-semibold mb-6">
              Contact Us
            </p>

            <div className="flex flex-col gap-5">
              {/* Address */}
              <div className="flex gap-3 items-start">
                <div className="w-8 h-8 border border-[#cbc7bc] flex items-center justify-center flex-shrink-0 mt-0.5 text-[#3a6349]">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-4 h-4">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                    <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
                  </svg>
                </div>
                <p className="text-[13px] text-black leading-relaxed">{contactInfo.address}</p>
              </div>

              {/* Phone */}
              <div className="flex gap-3 items-center">
                <div className="w-8 h-8 border border-[#cbc7bc] flex items-center justify-center flex-shrink-0 text-[#3a6349]">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-4 h-4">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
                  </svg>
                </div>
                <a href={`tel:${contactInfo.phone}`} className="text-[13px] text-black hover:text-[#3a6349] transition-colors">
                  {contactInfo.phone}
                </a>
              </div>

              {/* Email */}
              <div className="flex gap-3 items-center">
                <div className="w-8 h-8 border border-[#cbc7bc] flex items-center justify-center flex-shrink-0 text-[#3a6349]">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-4 h-4">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                  </svg>
                </div>
                <a href={`mailto:${contactInfo.email}`} className="text-[13px] text-black hover:text-[#3a6349] transition-colors">
                  {contactInfo.email}
                </a>
              </div>

              {/* Hours */}
              {/* <div className="flex gap-3 items-center">
                <div className="w-8 h-8 border border-[#cbc7bc] flex items-center justify-center flex-shrink-0 text-[#3a6349]">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-4 h-4">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <p className="text-[13px] text-[#555]">{contactInfo.hours}</p>
              </div> */}
            </div>
          </div>

        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-[#d8d4c8]">
        <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[11.5px] text-[#999]">
            © {new Date().getFullYear()} Forest Vibes. All rights reserved.
          </p>
          <div className="flex gap-6">
            {["Privacy Policy", "Terms of Service", "Cookie Policy"].map((item) => (
              <a key={item} href="#" className="text-[11.5px] text-[#999] hover:text-[#3a6349] transition-colors">
                {item}
              </a>
            ))}
          </div>
        </div>
      </div>
    </footer>
  );
}